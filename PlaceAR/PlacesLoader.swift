...
struct PlacesLoader {
  ...
  func loadPOIS(location: CLLocation, radius: Int = 30, handler: @escaping (NSDictionary?, NSError?) -> Void) {
    ...
    let uri = apiURL + "nearbysearch/json?location=\(latitude),\(longitude)&radius=\(radius)&sensor=true&places=restaurant&key=\(apiKey)"
    
    
    // start of Alamofire
    Alamofire.request(uri, method: .get, parameters: nil, encoding: URLEncoding(destination: .queryString)).responseJSON { (response) in
      if let data = response.result.value {
        print (data)
        do {
          let responseObject = data as! [String:Any]
